...
2. Для каждого кандидата вычисли `similar_to_n` (id похожего прошлого поста или `null`). Кандидат считается **похожим**, если выполняется любое из:
   ...
   - **(если активный BRAND.md содержит секцию `## Visual angles`)** тот же `lead_visual_angle` что у одного из последних 2 постов И ≥1 общий `noun` (визуально соседний пост — раздел «Visual angle pipeline» ниже).
...
2. **Visual angle (auto)** — если активный `BRAND.md` содержит секцию `## Visual angles`, Skill **сам** выбирает один angle на каждый слайд (без вопросов пользователю). Алгоритм — раздел «Visual angle pipeline» ниже.
3. **Inventory (auto)** — если активный `BRAND.md` содержит секцию `## Visual subject anchors`, Skill **сам** вытягивает 1–3 предмета на каждый слайд из inventory clusters; обязательно из **разных** кластеров внутри одного слайда; внутри карусели приоритет неиспользованным кластерам. Алгоритм — раздел «Inventory rotation» ниже.
4. **Visual scene** — конкретное описание сцены (environment, subject crop, pose, mood, props). Per-carousel композиция, живёт ТОЛЬКО в brief.md (см. CLAUDE.md → Layer isolation MUST §3). STYLE.md описывает только рендеринг (камера/свет/grade/grain), не сцену. Сцена пишется как развёртка выбранного `visual_angle` (его scene template из BRAND) с подставленным `inventory`.
5. Если `has_text=true`: заполняй **per-slide поля из `per_slide_schema` активной THEME** (frontmatter THEME.md). Skill **не** перечисляет имена полей сам — они theme-specific (одна тема ожидает eyebrow+headline+sub+rule, другая ожидает только headline+stat, etc.). Какие поля обязательные / опциональные / какие inline-spans разрешены внутри headline — всё в `per_slide_schema`.
6. `decor[]` — массив декор-элементов (≤3) **только** для слайдов с акцентом на число / шаги / чек-лист / стрелку до→после. Для каждого: `slug` (kebab-case), `purpose` (зачем), `placement` (rough position на холсте). Правила генерации — раздел «Декор» выше.

Записываем всё это в brief.md под каждым слайдом — см. шаблон ниже (поля `Visual angle:`, `Inventory:`, `Visual variables:`, `Decor:`).

### Visual angle pipeline  *(brand-conditional, опциональная фича)*

Активна **только** если `BRAND.md` содержит секцию `## Visual angles` (закрытый список ракурсов с scene templates). Бренды без этой секции — Skill работает по-старому (Visual variables пишутся вручную, без angle), backwards-compatible.

**Role → angle compatibility map** (универсальная семантика ролей; конкретные angle-имена приходят из активного BRAND.md):

| Role | Допустимые angle-семантики (ищи матч в `BRAND.md → ## Visual angles`) |
|---|---|
| `Hook` | hero / archival-flatlay / outdoor-pavement (любой angle с сильным визуальным entry) |
| `Recognition` | home-training / locker-corner / data-still-life (контекстный) |
| `Mechanic` | home-training / single-hero (показ действия / предмета) |
| `Stat`, `Proof` | data-still-life / archival-flatlay (про числа / артефакты) |
| `Receipt` | archival-flatlay / data-still-life / cafe-recap (sport receipt mood) |
| `Reframe` | gear-flatlay-floor / outdoor-pavement (визуальный shift) |
| `CTA` | single-hero / outdoor-pavement (закрывающий кадр) |
| `State`, `Pattern`, `Product` | подбирай по семантике; если не очевидно — `single-hero` |

Skill матчит закрытый список из BRAND по семантике (имена в BRAND могут отличаться — `morning-pavement` вместо `outdoor-pavement` и т.д.); используй contains / kebab-case проверку.

**Per-slide pick algorithm:**

1. Для слайда с ролью R получи множество допустимых angle-семантик из таблицы выше.
2. Найди в `BRAND.md → ## Visual angles` все angle, чьё имя содержит хотя бы одну из этих семантик. Это `candidates`.
3. Исключи angle, уже использованный в **предыдущем** слайде этой карусели (нет двух подряд одинаковых).
4. Исключи angle, уже использованный **дважды** в этой карусели (max 2 раза на 6 слайдов).
5. Если после фильтров `candidates` пустой — расслабь правило 4 (max 3 раза); если всё ещё пусто — игнорируй правило 3.
6. Из оставшихся выбери первый по порядку появления в `BRAND.md`.

**Lead angle** (per-carousel):

- `lead_visual_angle = visual_angle слайда 1 (Hook)`.
- При выборе angle для Slide 1 дополнительно прочитай `result/<combo_id>/history.jsonl`. Возьми `lead_visual_angle` последних 2 постов; **исключи** их из candidates для Slide 1. Если все доступные angle для Hook исчерпаны — возьми тот, что использовался давнее всего.
- `lead_visual_angle` записывается в `meta.json` на верхнем уровне и в `history.jsonl` (см. schema bumps).

### Inventory rotation  *(brand-conditional, опциональная фича)*

Активна **только** если `BRAND.md` содержит секцию `## Visual subject anchors` с `### Inventory clusters`.

**Per-slide pick algorithm:**

1. Прочитай BRAND inventory clusters (например: `running, strength, recovery, mobility, data, apparel`).
2. Для каждого слайда выбери 1–3 предмета **из разных кластеров**. Никогда не бери два предмета из одного кластера в одном слайде.
3. Внутри карусели веди счётчик `cluster_use_count`. Приоритет — кластеры с `count == 0`, затем `count == 1`, и т.д.
4. Apparel & body cues — особый кластер: используется на всех слайдах с body crop, не считается «новым» кластером для diversity.
5. Записывай выбранные предметы и их кластеры в `meta.json → slides[k].inventory_clusters[]` и в brief.md под слайдом (`Inventory:`).
...
```json
{"n":1,"created_at":"YYYY-MM-DD","topic_id":"thumb-muscle-memory","pillar":"<pillar>","hook_shape":"recognition-loop","hook_text":"<hook>","nouns":["a","b","c"],"slide_roles":["Hook","Recognition","Mechanic","Stat","Reframe","CTA"],"lead_visual_angle":"<angle slug или null>","visual_angles":["<per-slide angle slug или null>",...]}
```

Поля `lead_visual_angle` и `visual_angles[]` — опциональные; присутствуют только если активный BRAND.md описывает `## Visual angles`. На постах от брендов без angle-секции эти поля = `null` или отсутствуют. Старые записи без этих полей читаются без ошибки.
...
- **Lead visual angle:** <slug из BRAND.md → ## Visual angles> (опционально; есть только если BRAND описывает angles)
- **Nouns:** [a, b, c]
...
### Slide 1 — <role>
- **Has text:** yes/no
- **Visual angle:** <slug из BRAND.md → ## Visual angles> (auto, опционально — только если BRAND описывает angles)
- **Inventory:** [item1 (cluster), item2 (cluster), …] (auto, опционально — только если BRAND описывает Visual subject anchors)
...
- **Visual variables (для image-промпта):** environment=..., subject crop / pose=..., mood=..., palette hint=...
  *(STYLE.md задаёт только recipe рендеринга — что именно в кадре пишется здесь. Если активный BRAND описывает Visual angles — Visual variables развёртывают scene template выбранного angle с подставленными inventory items.)*
...
<environment, subject crop, pose, mood, palette hint, написанные в brief.md;
если активный BRAND описывает Visual angles — Visual variables развёртывают
scene template выбранного `visual_angle` с подставленным `inventory`>

Avoid: <NEGATIVE PROMPT блок из STYLE.md — только не-стиль>
       + <brief-level negative, если есть>
       + <BRAND.md → ## Visual forbidden — wellness drift, целиком, если секция есть>
       + <BRAND.md → ## Visual forbidden — competitor wordmarks, целиком, если секция есть>
```
...
**Brand-level forbidden** (`## Visual forbidden — wellness drift` и `## Visual forbidden — competitor wordmarks` в BRAND.md) инжектится в `Avoid:` блок **каждого** слайд-промпта **поверх** STYLE NEGATIVE. Это допустимо по Layer isolation — это brand-смысл (что бренд НЕ хочет показывать), не rendering recipe. Для брендов без этих секций — Skill ничего не добавляет, работает по-старому.

**Subject layer (кто в кадре, faceless / portrait, demographics) пока не введён.** До его появления skill ничего не инжектит про субъекта; если пользователь хочет конкретного субъекта — пишет в brief сам, либо через BRAND `## Visual subject anchors → Apparel & body cues` (опциональная секция).
...
```json
{
  ...
  "lead_visual_angle": "<angle slug или null>",
  "slides": [
    {
      "n": 1,
      "text": true,
      ...
      "layout": "bottom",
      "uses_accent": true,
      "visual_angle": "<angle slug из BRAND.md или null>",
      "inventory": [
        { "item": "<short label>", "cluster": "<cluster slug from BRAND>" }
      ],
      "inventory_clusters": ["<cluster slug>", "..."],
      "theme_fields": {
...
Поля `lead_visual_angle`, `slides[].visual_angle`, `slides[].inventory[]`, `slides[].inventory_clusters[]` — опциональные. Заполняются только если активный BRAND.md содержит секции `## Visual angles` / `## Visual subject anchors`. Иначе — `null` или отсутствуют.
...
6. **Step 5 — Per-slide design.** Auto-pick `visual_angle` per slide и `lead_visual_angle` (если активный BRAND содержит `## Visual angles`); auto-pick `inventory` per slide (если BRAND содержит `## Visual subject anchors`); затем Visual scene (развёртка scene template c подставленным inventory) + headline/sub + decor (если уместно). Без вопросов пользователю — алгоритмы детерминированные (см. «Visual angle pipeline» / «Inventory rotation»).
...
8. **Бриф.** Собрать `brief.md` по новому шаблону (Combo / Topic id / Hook shape / Angle / Lead visual angle / Nouns / Considered & rejected + секция `## Slide structure` + per-slide `Visual angle` + `Inventory` + `Visual variables`). Без `brief.md` к шагу 9 не переходим.
9. **Картинки.** Для каждого слайда → собрать prompt (STYLE PREAMBLE + сцена из brief Visual variables; `Avoid:` = STYLE NEGATIVE + BRAND `## Visual forbidden — wellness drift` + BRAND `## Visual forbidden — competitor wordmarks` если эти секции есть) → отдать активному провайдеру → сохранить в `assets/slide-K.png`. Декор — отдельным проходом (см. «Декор»).
...
13. **Pre-publish checklist:**
    ...
    - **Visual-angle diversity** *(только если активный BRAND содержит `## Visual angles`)*: никакие два соседних слайда не имеют одинаковый `visual_angle`; ни один angle не использован более 2 раз в карусели; `lead_visual_angle` ≠ `lead_visual_angle` ни одного из последних 2 постов комбинации.
    - **Visual anchors / forbidden** *(только если активный BRAND содержит `## Visual subject anchors` / `## Visual forbidden`)*: каждый слайд с `has_text=true` визуально содержит ≥1 предмет из BRAND `## Visual subject anchors` (валидируется через Read 1–2 финальных PNG); ни один слайд не содержит ничего из BRAND `## Visual forbidden — wellness drift`; нет читаемых wordmarks из BRAND `## Visual forbidden — competitor wordmarks`.
    - Чеклист валидируется по содержимому `inst/`.
14. **Append history.** Допиши новую строку в `result/<combo_id>/history.jsonl` с полями `{n, created_at, topic_id, pillar, hook_shape, hook_text, nouns, slide_roles, lead_visual_angle, visual_angles}`. Поля `lead_visual_angle` и `visual_angles[]` опциональные — заполняй только если активный BRAND описывает `## Visual angles`. Это обязательный шаг — без него следующая карусель не сможет сделать Novelty check.
